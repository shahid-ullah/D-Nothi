...
def nothi_users_male(request):
    load_users_gender_male_table()
    users_male_df = settings.USERS_GENDER_MALE_TABLE_CSV_FILE_PATH
    users_male_df_year_by = users_male_df.groupby('year')
    ...
    for year, year_frame in users_male_df_year_by:
        year = str(year)
        # year, year_frame.shape
        t_dict_ge = {'name': year, 'y': year_frame.shape[0], 'drilldown': year}
        general_series[0]['data'].append(t_dict_ge)
