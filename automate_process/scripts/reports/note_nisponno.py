...
from dashboard_generate.models import ReportGenerationLog, ReportNoteNisponnoModel
...
def format_and_load_to_mysql_db(request, *args, **kwargs):
    ...
    for office_id, report_date, note_nisponno_count in zip(
        grouped_report_date['office_id'].values,
        grouped_report_date['report_date'].values,
        grouped_report_date['size'].values,
    ):
        object_dict = generate_model_object_dict(request, report_date, note_nisponno_count, office_id, *args, **kwargs)
...
def get_nisponno_records_querysets(*args, **kwargs):
    ...
    if ReportNoteNisponnoModel.objects.exists():
